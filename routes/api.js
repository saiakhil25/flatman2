...
module.exports = function(router, upload) {
	...
	router.post('/upload-image', function(req, res) {
		newUpload(req, res, function(error) {
			if(error) {
				res.json({success: false, error: "File too large, maximum size 1 MB"});
			}
			else {
				var newObject = {};
				newObject["avatar"] = req.file.path;
				if(req.body.model == 'user') {
					User.findByIdAndUpdate(req.body.id, {$set: newObject}, function(err) {
						if(err) {
					...
					Customer.findByIdAndUpdate(req.body.id, {$set: newObject}, function(err) {
						if(err) {
							res.json({success: false, error: err.stack});
						}
						else {
					...
					Patient.findByIdAndUpdate(req.body.id, {$set: newObject}, function(err) {
						if(err) {
							res.json({success: false, error: err.stack});
						}
						else {
					...
					Doctor.findByIdAndUpdate(req.body.id, {$set: newObject}, function(err) {
						if(err) {
							res.json({success: false, error: err.stack});
						}
						else {
					...
					FOS.findByIdAndUpdate(req.body.id, {$set: newObject}, function(err) {
						if(err) {
							res.json({success: false, error: err.stack});
						}
						else {
