	...
	router.get('/doctors/all', function(req, res) {
		...
		async.waterfall([
			...
			function(doctors, callback) {
				Specialization.find({}, function(err, specializations) {
					if(err)
						...
						callback(null, {doctors: doctors, specializations: specializations})
				})
			},
			function(dataObject, callback) {
				var doctorsArray = [];
				for (var i = 0; i < dataObject.doctors.length; i++) {
					var specializationName = _.filter(dataObject.specializations, function(spec) {
			...
			if(err)
				...
				res.json({success: true, data: doctorsInfo})
		})
	});
								...
								addDoctor(req.body, function(response) {
									if(response.success) {
										...
										deleteUser(user._id, function(resp) {
											if(response.success) {
												res.json({success: false, message: "Error in creating doctor.", error: err.stack});
											}
											else {
