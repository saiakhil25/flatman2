...
module.exports = function(router) {
    ...
    router.get('/get-assigned-fos/:patientId', function(req, res) {
        ...
        async.waterfall([
            function(callback) {
                AssignFOS.find({patientId: req.params.patientId}, function(err, assignedFosList) {
                    if(err) {
                        callback(err)
                    }
                    else {
    ...
    router.get('/get-assigned-doctor/:patientId', function(req, res) {
        ...
        async.waterfall([
            function(callback) {
                AssignDoctor.find({patientId: req.params.patientId}, function(err, assignedDocList) {
                    if(err) {
                        callback(err)
                    }
                    else {
    ...
    router.get('/get-assigned-patient-doc/:docId', function(req, res) {
        async.waterfall([
            function(callback) {
                AssignDoctor.find({doctorId: req.params.docId}, function(err, assignPatientList) {
                    if(err) {
                        callback(err);
                    }
                    else {
    ...
    router.get('/get-assigned-patient-fos/:fosId', function(req, res) {
        async.waterfall([
            function(callback) {
                AssignFOS.find({fosId: req.params.fosId}, function(err, assignPatientList) {
                    if(err) {
                        callback(err);
                    }
                    else {
