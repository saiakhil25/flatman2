app.controller('PatientController', function ($rootScope, $scope, $http, $state, $stateParams, $rootScope, $modal, $sce) {
	...
	var refresh = function() {
		...
		if($rootScope.user.userRole == 2) {
			...
			$http.get('/api/get-assigned-patient-doc/' + $rootScope.user.doctorId).success(function(response) {
			...
			$scope.patients = [];
			$http.get('/api/get-assigned-patient-fos/' + $rootScope.user.fosId).success(function(response) {
				for (var i = 0; i < response.data.length; i++) {
					$scope.patients.push(response.data[i].patientInfo);
				}
				$scope.gridOptions.data = $scope.patients;
		...
		if($stateParams.patientId !== "undefined") {
			$http.get('/api/patient/get-patient-by-id/' + $stateParams.patientId).then(function(response) {
		...
		$rootScope.currentModal = $modal.open({
			...
			controller: function($scope, $http, $modalInstance, $state, $stateParams, $rootScope) {
				...
				$scope.createAnalysis = function() {
					...
					$http.post('/api/analysis/create', $scope.analysis).success(function(response) {
						$scope.showAlert = true;
						if(response.success) {
							$scope.alert = {status: "success", message: response.message};
							$state.reload();
