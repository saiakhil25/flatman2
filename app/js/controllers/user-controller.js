...
angular.module('xenon.customControllers', []).
	controller('UserController', function($scope, $rootScope, $modal, $sce, $http, $state, $stateParams) {
		...
		var refresh = function() {
			...
			if($stateParams.userId !== undefined) {
				$http.get('/user/' + $stateParams.userId)
				...
				controller: function($scope, $http, $modalInstance, $state) {
						...
						$scope.createUser = function() {
							...
							.success(function(response) {
								if(response.success) {
									refresh();
									$scope.showAlert = true;
									$scope.alert = {message: response.message, status: 'success'};
									$scope.user = {};
									$state.reload();
								}
								else {
					...
					if(isDelete.status) {
						$scope.deleteUser = function() {
							$http.delete('/user/delete/' + isDelete.id).then(function(response) {
								if(response.data.success) {
									refresh();
									$state.reload();
									$modalInstance.close();
